...
Ex08_21::Ex08_21()
	: OGLWindow("Example08_21", "Example 8.21 (M)")
{
}
...
void Ex08_21::InitGL()
{
	...
	ShaderInfo  object_shaders[] = {
		...
		{ GL_FRAGMENT_SHADER, "Shaders/sh08_21.frag" },
		{ GL_NONE, NULL }
	};
	...
	GLuint LightWood_loc = glGetUniformLocation(object_prog, "LightWood");
	GLuint DarkWood_loc = glGetUniformLocation(object_prog, "DarkWood");
	GLuint RingFreq_loc = glGetUniformLocation(object_prog, "RingFreq");
	GLuint LightGrains_loc = glGetUniformLocation(object_prog, "LightGrains");
	GLuint DarkGrains_loc = glGetUniformLocation(object_prog, "DarkGrains");
	GLuint GrainThreshold_loc = glGetUniformLocation(object_prog, "GrainThreshold");
	GLuint NoiseScale_loc = glGetUniformLocation(object_prog, "NoiseScale");
	GLuint Noisiness_loc = glGetUniformLocation(object_prog, "Noisiness");
	GLuint GrainScale_loc = glGetUniformLocation(object_prog, "GrainScale");
	...
	glUniform3fv(LightPosition_loc, 1, vmath::vec3(0.0f, 0.0f, 4.0f));
	glUniform1f(Scale_loc, 2.0f);

	glUniform3fv(LightWood_loc, 1, vmath::vec3(0.6f, 0.3f, 0.1f));
	glUniform3fv(DarkWood_loc, 1, vmath::vec3(0.4f, 0.2f, 0.07f));
	glUniform1f(RingFreq_loc, 4.0f);
	glUniform1f(LightGrains_loc, 1.0f);
	glUniform1f(DarkGrains_loc, 0.0f);
	glUniform1f(GrainThreshold_loc, 0.5f);
	glUniform3fv(NoiseScale_loc, 1, vmath::vec3(0.5f, 0.1f, 0.1f));
	glUniform1f(Noisiness_loc, 3.0f);
	glUniform1f(GrainScale_loc, 27.0f);
...
void Ex08_21::keyboard( unsigned char key, int x, int y )
{
	switch( key ) {
	case 'M': 
...
void Ex08_21::make3DNoiseTexture()
{
	int f, i, j, k, inc;
...
double Ex08_21::noise3(double *ni)
{
	const int i = 12;
	static int c = 0;
	static int cur = 0;
